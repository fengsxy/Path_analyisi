#!/usr/bin/env python3
"""Evaluate γ_B + Δλ DP path: compute path, generate images, compute FID."""
import sys
...
BATCH_SIZE = 64
SEED = 42
OUTPUT_DIR = 'results/gammaB_lambda'
...
print("=" * 60, flush=True)
print("γ_B + Δλ DP Path Evaluation", flush=True)
print(f"Epoch: {EPOCH}, Images: {NUM_IMAGES}, Device: {DEVICE_ID}", flush=True)
print("=" * 60, flush=True)

# 1) Load cached heatmap (with γ_B already applied)
cache_path = f'results/heatmap_30_ema_{EPOCH}ep.pth'
print(f"\nLoading heatmap: {cache_path}", flush=True)
data = torch.load(cache_path, map_location='cpu', weights_only=False)

snr_grid = data['snr_grid']
error_64_B = data['error_64']  # already has γ_B
error_32_B = data['error_32']  # already has γ_B
t_grid = data['t_grid']
log_snr = torch.log(snr_grid)

# 2) Compute DP path with Δλ step size (using γ_B error)
print("\nComputing γ_B + Δλ DP path (18 steps)...", flush=True)
path, cost = find_optimal_path_lambda(error_64_B, error_32_B, log_snr, num_steps=18)
...
# 6) Generate images
print(f"\nGenerating {NUM_IMAGES} images with γ_B + Δλ path...", flush=True)
gen_images_list = []
num_batches = (NUM_IMAGES + BATCH_SIZE - 1) // BATCH_SIZE
generated = 0

for _ in tqdm(range(num_batches), desc="Generating (γ_B+Δλ)"):
...
# Save grid
grid_path = os.path.join(OUTPUT_DIR, f'grid_gammaB_lambda_{EPOCH}ep.png')
...
print(f"\n{'=' * 60}", flush=True)
print(f"FID (γ_B + Δλ, EMA {EPOCH}ep): {fid:.4f}", flush=True)
print(f"[Reference] FID (γ_B + grid, EMA {EPOCH}ep): 29.45", flush=True)
print(f"[Reference] FID (γ_D + Δλ, EMA {EPOCH}ep): 35.17", flush=True)
print(f"{'=' * 60}", flush=True)

# Save result
with open(os.path.join(OUTPUT_DIR, 'results.txt'), 'w') as f:
    f.write(f"γ_B + Δλ DP Path Evaluation\n")
    f.write(f"Epoch: {EPOCH}, Images: {NUM_IMAGES}, Seed: {SEED}\n")
    f.write(f"FID (γ_B + Δλ): {fid:.4f}\n")
    f.write(f"FID (γ_B + grid, reference): 29.45\n")
    f.write(f"FID (γ_D + Δλ, reference): 35.17\n")
