#!/usr/bin/env python3
"""
Evaluate explore_test_5 models with DP paths (heatmap + optimal path + FID).

All 3 regimes use SingleTimestepModel with EMA weights.
...
Usage:
    python eval_dp.py --model_type same_t --epochs 200 400 --device 0
    python eval_dp.py --model_type dp_path --epochs 200 400 --device 1
    python eval_dp.py --model_type heuristic --epochs 200 400 --device 2
"""
...
def compute_error_heatmap(model, scheduler, x_batch, device='cpu', K=4):
    """Compute 30x30 error heatmap. Only error_64 (model doesn't predict 32x noise)."""
    num_points = 30
...
@torch.no_grad()
def generate_batch_with_path(model, scheduler, batch_size,
...
def compute_or_load_heatmap(checkpoint_path, model_type, epoch, device, output_dir,
    ...
    t_grid, snr_grid, error_64 = compute_error_heatmap(model, scheduler, x_batch, device=device, K=4)
    ...
    print(f"  DP: {len(samples)} points, {samples[0]} -> {samples[-1]}", flush=True)
    print(f"  t_64: {ts_64}", flush=True)
    print(f"  t_32: {ts_32}", flush=True)

    results = {
        't_grid': t_grid, 'snr_grid': snr_grid, 'error_64': error_64,
        'samples': samples, 'cost_n': cost_n,
...
def evaluate_dp_path(checkpoint_path, model_type, heatmap_data, feat_real, num_images,
    ...
    for _ in tqdm(range(num_batches), desc="Generating (DP)"):
        ...
        images = generate_batch_with_path(model, scheduler, bs,
...
def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate explore_test_5 models with DP paths')
...
def main():
    args = parse_args()
    print("=" * 50, flush=True)
    print("Explore Test 5 - DP Path Evaluation", flush=True)
    print("=" * 50, flush=True)
    ...
    for epoch in args.epochs:
        ...
        heatmap_check = os.path.join(args.output_dir, f'grid_{args.model_type}_dp_{epoch}ep.png')
        if os.path.exists(heatmap_check) and not args.force:
            print(f"[Skip] Already evaluated: {heatmap_check}", flush=True)
            continue
