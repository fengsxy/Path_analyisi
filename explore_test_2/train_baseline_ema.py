#!/usr/bin/env python3
"""
Training Script for Baseline Model with EMA on AFHQ64
...
Usage:
    python train_baseline_ema.py --epochs 2000 --batch_size 64 --device 0
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train Baseline with EMA on AFHQ64')
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if args.save_every is not None and (epoch + 1) % args.save_every == 0:
            # Save regular model
            checkpoint_path = os.path.join(args.output_dir, f'baseline_ema_{epoch+1}ep.pth')
            torch.save({
    ...
    # Save final model
    save_path = os.path.join(args.output_dir, 'baseline_ema_final.pth')
    torch.save({
        'model_state': model.state_dict(),
        'ema_state': ema.state_dict(),
        'optimizer_state': optimizer.state_dict(),
