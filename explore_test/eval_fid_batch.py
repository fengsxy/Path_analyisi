#!/usr/bin/env python3
"""
Batch evaluate FID for explore_test models (single_t, no_t) using diagonal path.

Key difference from main eval_fid_batch.py:
- x_32 is downsampled from x_64 each step (not random noise)
- Models only output noise_pred_64

Usage:
    python eval_fid_batch.py --model_type single_t --epochs 200 400 600 800 1000
    python eval_fid_batch.py --model_type no_t --epochs 200 400 600 800 1000
"""
...
@torch.no_grad()
def generate_batch_single_t(model, scheduler, batch_size, num_steps, device, eta=0.0):
    """Generate with single_t model. x_32 = downsample(x_64) each step."""
...
@torch.no_grad()
def generate_batch_no_t(model, scheduler, batch_size, num_steps, device, eta=0.0):
    """Generate with no_t model. x_32 = downsample(x_64) each step."""
    scheduler._set_timesteps(num_steps)
    timesteps = scheduler.timesteps
...
def load_real_features(num_images, device, cache_dir=None,
                       cache_path='../results/real_features.npz'):
...
def evaluate_checkpoint(checkpoint_path, model_type, feat_real, num_images, num_steps,
    ...
    model = load_model_from_checkpoint(checkpoint_path, model_type, device)

    if model_type == 'single_t':
        generate_fn = generate_batch_single_t
    else:
        generate_fn = generate_batch_no_t
...
def parse_args():
    parser = argparse.ArgumentParser(description='Batch evaluate FID for explore_test models')
...
def main():
    args = parse_args()
    print("=" * 50, flush=True)
    print("Explore Test - Batch FID Evaluation (Diagonal)", flush=True)
    print("=" * 50, flush=True)
