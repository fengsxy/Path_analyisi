...
Usage:
    python generate.py --checkpoint checkpoints/single_t_final.pth --output_dir results/single_t --num_images 1000
    python generate.py --checkpoint checkpoints/no_t_final.pth --output_dir results/no_t --num_images 1000
"""
...
def generate_batch_single_t(model, scheduler, batch_size, num_steps, device, eta=0.0):
    """Generate with single_t model (diagonal path for 32×32)."""
    ...
    for i, t in enumerate(timesteps):
        ...
        # 32×32 也按同样的 schedule 去噪（diagonal path）
        # 但这里我们没有 noise_pred_32，所以保持 x_32 不变或者重新采样
        # 简单起见：每步重新采样 32×32 noise
        x_32 = torch.randn(batch_size, 3, 32, 32, device=device)
...
def generate_batch_no_t(model, scheduler, batch_size, num_steps, device, eta=0.0):
    """Generate with no_t model (blind denoiser)."""
    scheduler._set_timesteps(num_steps)
    timesteps = scheduler.timesteps
...
def main():
    ...
    for batch_idx in tqdm(range(num_batches), desc="Generating"):
        ...
        images = generate_fn(model, scheduler, current_batch_size, args.num_steps, device, args.eta)
